...
;to create-connections
;  let candidates other turtles in-radius 影响半径
;  let connections min (list avg-degree count candidates)
;
;  while [count my-links < connections] [
;    let potential-link one-of candidates with [not member? myself [my-links] of myself]
;    if potential-link != nobody [
;      create-link-with potential-link
;    ]
;  ]
;end
...
;  repeat 10
;  [
;    layout-spring turtles links 0.3 (world-width / (sqrt 居民总数)) 1
;  ]
;end
...
"" ""
PENS
"中立态度居民人数" 1.0 0 -7500403 true "" "plot (count turtles with [not infected? and not resistant?]) / (count turtles) * 100"
"拒绝参与居民人数" 1.0 0 -13345367 true "" "plot (count turtles with [infected?]) / (count turtles) * 100"
"具有参与行为居民人数" 1.0 0 -2674135 true "" "plot (count turtles with [resistant?]) / (count turtles) * 100"
